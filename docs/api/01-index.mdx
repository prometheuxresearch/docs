...
- **MCP Integration**: Connect AI agents like Claude to your ontologies via Model Context Protocol
  - **Local MCP Server** (`prometheux-mcp`): For Claude Desktop users
  - **Remote MCP Server** (`px-remote-mcp-server`): For Claude Web users with OAuth
...
See the [full MCP documentation](/api/mcp) for detailed setup and usage.

  </TabItem>
  <TabItem value="mcp-remote" label="MCP (Claude Web)">

### Cloud-Hosted with OAuth

The remote MCP server works with Claude Web (browser-based) and uses OAuth for authentication.

### Setup

1. **Open Claude Web** at [claude.ai](https://claude.ai)
2. **Add Server:**
   - Settings → Developer → Model Context Protocol → Add Server
   - **Server URL:** `https://api.prometheux.ai/px-remote-mcp-server`
3. **Authenticate:**
   - Claude redirects you to the Prometheux login page
...
See the [full MCP documentation](/api/mcp#remote-mcp-server-claude-web) for detailed setup and troubleshooting.
...
| [**Applications**](/api/endpoints/applications) | Advanced applications like Graph RAG |
