...
Connect [Genie Code](https://www.databricks.com/blog/introducing-genie-code) to Prometheux using the [Remote MCP Server](/api/mcp/remote). Genie Code is Databricks' autonomous AI agent for data work — it supports [MCP as an extensibility mechanism](https://www.databricks.com/blog/introducing-genie-code), allowing it to securely interact with external tools and APIs.
...
:::tip Available Tools
See the [MCP overview page](/api/mcp#available-tools) for the complete list of tools exposed to Genie Code.
:::
