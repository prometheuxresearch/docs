...
6. Restart the kernel
  ...
  <TabItem value="mcp-local" label="MCP (Claude Desktop)">
...
> **Tip:** Run `which prometheux-mcp` to find the exact path on your system.
...
See the [full MCP documentation](/api/mcp/local) for detailed setup and usage.

  </TabItem>
  <TabItem value="mcp-remote" label="MCP (Remote)">
...
- **[Claude (Desktop & Web)](/api/mcp/clients/claude)** — Add as a custom connector in Claude settings
- **[Genie Code (Databricks)](/api/mcp/clients/databricks-genie)** — Register as an external MCP server

For other MCP clients, see the [Remote MCP Server](/api/mcp/remote#server-details) page for OAuth endpoints and connection parameters.
...
See the [full MCP documentation](/api/mcp/remote) for detailed setup and troubleshooting.
...
|----------|-------------|
| [**Config**](/api/config) | SDK configuration and LLM setup |
...
| [**Rule Inference**](/api/endpoints/rule-inference) | Schema inference from data sources |
| [**Dashboards**](/api/endpoints/dashboards) | Dashboard CRUD |
| [**Vadalingo**](/api/endpoints/vadalingo) | NL, SQL, RDF, and OWL to Vadalog translation |
| [**AI Agent**](/api/endpoints/agent) | Streaming conversational AI agent |
