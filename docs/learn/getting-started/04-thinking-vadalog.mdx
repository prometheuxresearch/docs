...
later on line 19), and moving the `Shares` variable into the head of the
predicate so that we can keep track of its value.
...
aggregation](../vadalog/expressions-operators#monotonic-aggregates) to the joint shares for any given intermediary to
...
To compose data in relational database management systems (RDBMS), you often need
to reach around the abstraction of business entities that tables provide. Answering
complex then requires unweildy joins across many different tables.

A simple way to think in Vadalog, is that multiple body predicates separated
by commas **are** indeed a join between each predicate:
...
<Tabs>
  ...
  <TabItem value="sql" label="SQL" default>
    ...
    SELECT 
      ...
      "owns"."owned" as 'SomeBrand',
      "owns"."shares",
      FROM "owns" INNER JOIN "controls"
      ON "controls"."controllee" = "owns"."owner";
    ```
  </TabItem>
</Tabs>

But instead of writing the actual tables 
