...
<Tabs>
  <TabItem value="simple" label="Simple Config" default>

If `prometheux-mcp` is in your PATH (after `pipx ensurepath`):
...
```
  ...
  <TabItem value="fullpath" label="Full Path Config">

If you need to specify the full path (find it with `which prometheux-mcp`):
...
```
   ...
   :::tip
   Replace `YOUR_USERNAME` with your actual username, or run `which prometheux-mcp` to find the exact path.
   
   **Advanced:** If you need to specify the complete JarvisPy path (e.g., for custom routing), include it in the URL:
   ```
...
This usually means Claude Desktop can't access the installed package.
...
The `prometheux-mcp` command isn't in your PATH.

**Solution:**
1. Run `pipx ensurepath` to add pipx binaries to your PATH
2. Restart your terminal or log out and back in
3. Restart Claude Desktop
4. Alternatively, use the full path: `which prometheux-mcp` and update your config
...
- **REST API:** See [REST API Reference](/api/endpoints) for direct HTTP access

## Related Resources

- [Concepts](/api/endpoints/concepts) - Learn about the concept API
