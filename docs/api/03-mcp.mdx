...
:::info Why pipx?
pipx installs the package in an isolated environment and makes the command available globally. This works reliably with Claude Desktop on all platforms, especially macOS where GUI apps may have PATH issues.
:::

  </TabItem>
  <TabItem value="pip" label="pip (Windows/Linux Only)">

```bash
pip install prometheux-mcp
```

:::warning Not Recommended for macOS
On macOS, Claude Desktop may have permission issues accessing pip-installed packages. Use pipx instead (see previous tab).
:::
...
<Tabs>
  <TabItem value="simple" label="Standard Configuration" default>
...
:::tip Automatic URL Construction
...
:::
  ...
  <TabItem value="fullpath" label="Full Path (For Troubleshooting)">

If you get "command not found" or "Server disconnected" errors, use the full path:
...
- **macOS/Linux:** Run `which prometheux-mcp` in your terminal
- **Windows:** Run `where prometheux-mcp` in PowerShell or Command Prompt

**Windows example:** `C:\\Users\\YOUR_USERNAME\\AppData\\Local\\Programs\\Python\\Python310\\Scripts\\prometheux-mcp.exe`  
**macOS example:** `/Users/YOUR_USERNAME/.local/bin/prometheux-mcp`
